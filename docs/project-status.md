...
在不修改 Codex Desktop 安装包的前提下，为 Codex Desktop 增加一个本地旁路监测页，用于查看更详细的 app-server 会话流。
...
- 尽量不影响 Desktop 正常使用；配置、日志目录、viewer 或持久化失败时降级为纯透传。
...
- 右侧正文和 detail 保留聚合后的消息内容以及有界原始样本；超长内容可能截断。
- 项目源码目录：
...
- 左侧 turn title 可以精简。
- 右侧 conversation 正文、Segment detail、Full info 不做导航级摘要，但仍受内容上限、采样和截断策略约束。
- wrapper exe 默认安装到 `C:\Users\<user>\Documents\CodexTrace\bin\codex-trace-wrapper.exe`，运行时配置安装到同目录 `C:\Users\<user>\Documents\CodexTrace\bin\config.toml`。`CODEX_CLI_PATH` 和 `CODEX_TRACE_WRAPPER_CONFIG` 分别指向这两个路径。受管理设备仍可能要求签名或管理员审批。
